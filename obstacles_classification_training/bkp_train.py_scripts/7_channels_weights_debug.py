...
def df_generator(gen, dataset, dataframe_dir, target_size, batch_size, class_mode, subset):
    ...
    while True:
        ...
        if subset == 'validation':
            filenames = im_gen.filenames
            for j, f in enumerate(filenames[:3]):
                print(f'==== Validation: filename = {f} ==============')
                print(f'==== Validation: label = {labels[j]} ==============')
                print(f'==== Validation: sample_weight = {sample_weights[j]} ==============')
