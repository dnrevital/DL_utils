...
def get_model(input_shapes, reg, metrics):
    ...
    x = Conv2D(16, (3, 3), padding='same', activation = tf.keras.layers.LeakyReLU(alpha=lrelu_alpha))(inp)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(2, 2), padding='same')(x)

    x = Conv2D(32, (5, 5), padding='same', activation = tf.keras.layers.LeakyReLU(alpha=lrelu_alpha))(x)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(2, 2), padding='same')(x)

    x = Conv2D(64, (5, 5), padding='same', activation = tf.keras.layers.LeakyReLU(alpha=lrelu_alpha))(x)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(2, 2), padding='same')(x)

    x = Conv2D(128, (5, 5), padding='same', activation = tf.keras.layers.LeakyReLU(alpha=lrelu_alpha))(x)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(2, 2), padding='same')(x)

    x = Conv2D(256, (5, 5), padding='same', activation = tf.keras.layers.LeakyReLU(alpha=lrelu_alpha))(x)
    x = BatchNormalization()(x)
    ...
    # Setting a weight-deacy frunction for the ADAMW optimizer
    step = tf.Variable(0, trainable=False)
    schedule = tf.optimizers.schedules.PiecewiseConstantDecay([3500, 7000], [1e-0, 1e-1, 1e-2])
...
def three_im_generator(gen, dataset, target_size, batch_size, class_mode, subset):
...
def main(args):
    ...
    training_generator = three_im_generator(
        datagen,
        training_dataset,
    ...
    validation_generator = three_im_generator(
        datagen,
        training_dataset,
        target_size=(img_height, img_width),
        batch_size=batch_size,
    ...
    eval_generator = three_im_generator(
        datagen,
        eval_dataset,
        target_size=(img_height, img_width),
        batch_size=batch_size,
