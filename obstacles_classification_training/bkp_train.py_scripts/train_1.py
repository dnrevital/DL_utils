...
class CustomDataGenerator(ImageDataGenerator):
    def __init__(self, p=0.25, debug_images=True, **kwargs):
        self.imcount = 0
        ...
        self.augments = {0: [r, b, g],
                         1: [g, r, b],
                         2: [g, b, r],
                         3: [b, r, g],
                         4: [b, g, r]} 
        
        super().__init__(preprocessing_function=self.my_augments, **kwargs)
        
    def my_augments(self, image):
        rand = random.randint(1, 100)
        if rand <= self.p:
            print(f'================== my_augments start ===================')
            im = np.array(image)
            aug = rand % 5
            height = im.shape[0]
            width = im.shape[1]
            mixed = [[[im[i][j][self.augments[aug][k]] for k in range(3)]
                                for j in range(width)] for i in range(height)]
            mixed = np.array(mixed)
            if self.debug_images:
                self.upload_image(mixed)
            return mixed
            print(f'================== my_augments end ======================')
        return image
...
def dir_generator(gen, dataset, target_size, batch_size, class_mode, subset):
    ...
    while True:
        ...
        for im in images:
            ...
            #im1 = cv2.resize(im1, dsize=comp_target_size, interpolation=cv2.INTER_LINEAR)
            #im2 = cv2.resize(im2, dsize=comp_target_size, interpolation=cv2.INTER_LINEAR)
            #im3 = cv2.resize(im3, dsize=comp_target_size, interpolation=cv2.INTER_NEAREST)
...
def main(args):
    ...
    datagen = CustomDataGenerator(validation_split=0.2)
