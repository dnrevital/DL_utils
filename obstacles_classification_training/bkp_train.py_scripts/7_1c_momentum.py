...
def main(args):
    ...
    # Build lr_values for the one cycle lr
    total_steps = epochs * steps_per_epoch
    final_steps = int(0.05 * total_steps)
    cycle_steps = total_steps - final_steps
    cycle_1st_half_steps = cycle_steps//2
    cycle_2nd_half_steps = cycle_steps - cycle_1st_half_steps
    lr_1st_half_values = np.linspace(min_lr, max_lr, cycle_1st_half_steps)
    lr_2nd_half_values = np.linspace(max_lr, min_lr, cycle_2nd_half_steps)
    lr_final_values = np.full((final_steps), min_lr)
    lr_values = np.concatenate((lr_1st_half_values, lr_2nd_half_values, lr_final_values))
    momentum_1st_half_values = np.linspace(max_momentum, min_momentum, cycle_1st_half_steps)
    momentum_2nd_half_values = np.linspace(min_momentum, max_momentum, cycle_1st_half_steps)
    momentum_final_values = np.full((final_steps), max_momentum)
    momentum_values = np.concatenate((momentum_1st_half_values, 
                                      momentum_2nd_half_values,
                                      momentum_final_values))
    ...
    class MyCallback(tf.keras.callbacks.Callback):
        ...
        def on_batch_begin(self, batch, logs=None):
            K.set_value(model.optimizer.lr, self.lr_values[self.step])
            K.set_value(model.optimizer.momentum, self.momentum_values[self.step])
            self.step += 1
            
        def on_epoch_end(self, epoch, logs=None):
            print(f'lr: {K.get_value(self.model.optimizer.lr):.8f}')
            print(f'momentum: {K.get_value(self.model.optimizer.momentum):.8f}')
            checkpoint_filepath = f'{checkpoint_prefix}/{model_name}/{model_time}/epoch-{epoch+1}'
            model.save(checkpoint_filepath, save_format='tf')
            
    my_callback = MyCallback(lr_values)
